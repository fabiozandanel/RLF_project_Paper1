...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
  {\bf We construct a phenomenological gas density model for the intra-cluster medium
  based on X-ray inferred densities for cool-core and non cool-core clusters, and a 
  cluster-mass-dependent gas fraction. We then build a mock galaxy cluster 
  catalog from the large MultiDark $N$-body $\Lambda$CDM simulation by assigning
  such a (mass-dependent) gas density profile to each dark matter halo in our sample, 
  which is sorted into cool-core and non cool-core clusters according to a dynamical 
  disturbance parameter. Our gas density model permits to match the observed 
  Sunyaev-Zel'dovich and X-ray scaling relations and luminosity function. Therefore,
  it can be reliable applied to make predictions for present and future instruments. 
  We apply our catalogs to calculate non-thermal radio and gamma-ray emission of galaxy 
  clusters in a companion paper. Eventually, we make our cosmologically complete multi-frequency 
  mock catalogs for the (non-)thermal cluster emission at different redshifts publicly and freely 
  available on-line through the MultiDark database.}
\end{abstract}
...
{\bf The study of the (non-)thermal emission in galaxy clusters provides
...
The simplest approach is to assume that clusters are self-similar with 
the thermal properties determined by their mass \citep{1986MNRAS.222..323K}.
However, X-ray observations showed that clusters are extremely 
complex systems. In fact, we know that self-similarity is broken, especially at 
low-mass scales (see \citealp{2005RvMP...77..207V} for a review).
The study of the thermodynamical condition of clusters is crucial 
to understand how self-similarity is broken. One possible solution 
is, for example, energy feedback from active galactic nuclei (e.g.,
\citealp{2013arXiv1303.5443P}). 

In order to build a cluster sample with realistic ICM densities, we 
adopt a \emph{phenomenological} approach and construct our density 
profiles from X-ray observations. We then compare the characteristics of 
the clusters in our mock sample to observational results. We study the X-ray 
luminosity function (XLF), the luminosity-mass relation, $L_{\rmn{X}}- M$, and 
the $Y_{\rmn{X}}-M$ relation, where $Y_{\rmn{X}}=M_\rmn{gas}k_{\rmn{B}}T$ 
with an X-ray-derived gas mass $M_{\rmn{gas}}$ and temperature $T$
\citep{2006ApJ...650..128K}. Moreover, we consider the thermal 
Sunyaev-Zel’dovich (SZ) effect and compare our $Y_{\rmn{SZ}}-M$ 
relation with recent SZ-derived measurements. Only if our model matches
available cluster data on the gas properties, we can safely use it in Paper~II
and be sure that we will obtain realistic predictions.
The final result is a complete cosmological cluster mock catalog, at
different redshifts, containing information on the DM and ICM properties 
of each object, together with its (non-)thermal emission at different 
frequencies. This mock catalog reproduces existing observations
and is useful in making predictions for future surveys.

In next Section~\ref{sec:2}, we introduce the MultiDark simulation
along with our final cluster sample. We assign to each of our clusters  
the phenomenologically constructed gas density profile in 
Section~\ref{sec:3}. In Sections~\ref{sec:4} and 
\ref{sec:5}, we show that this approach can successfully reproduce 
the known X-ray and SZ cluster characteristics. We present the
resulting mock cluster catalogs in Section~\ref{sec:6}, and eventually 
summarize our conclusions in Section~\ref{sec:7}.
In this work, the cluster mass $M_{\Delta}$ and radius $R_{\Delta}$ are defined 
with respect to a density that is $\Delta=200$ or $\Delta=500$ times the \emph{critical} 
density of the Universe. We adopt density parameters of $\Omega_{\rmn{m}}=0.3$, 
$\Omega_{\Lambda}=0.7$ and today's Hubble constant of 
$H_0 = 100 \times h_{70}$~km~s$^{-1}$~Mpc$^{-1}$ where $h_{70} = 0.7$.
}
