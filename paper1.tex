...
\begin{abstract}
  {\bf Cosmological hydrodynamical simulations of galaxy clusters are still
    ...
    on a representative sample of \qc{observed} X-ray clusters. We create a mock galaxy
    cluster catalog \qc{based on the large collisionless $N$-body simulation MultiDark, by 
    assigning our gas density model to each dark matter cluster halo.}
    %by assigning our gas density model to each cluster halo of
    %the large collisionless $N$-body simulation MultiDark. 
    Our clusters \qc{are classified as cool-core and non cool-core} according to a
    ...
    as the X-ray luminosity function, thus enabling \qc{to build} a reliable mock catalog for
    present surveys and forecasts for future \qc{experiments}. In a companion paper,
...
{\bf Galaxy clusters are the rarest and largest gravitationally-collapsed
  ...
  core clusters (CCCs)\qc{, while} others have cooling times that can be longer than
  ...
  \citep{2007ARA&A..45..117M, 2012NJPh...14e5023M}\qc{, owing} to the self-regulated
  ...
  \qc{comparison between} data and simulations with AGN feedback improved for integrated
  ...
  (SZ) data with the goal to facilitate \qc{comparison with} future hydrodynamical
  simulations, to allow the modeling of non-thermal cluster emission over the
  entire electromagnetic \qc{wave-band}, and finally to enable the construction of
  ...
  simulations. In this work, we make use of the recent \qc{large-volume} $N$-body
  simulation MultiDark, which employs a \qc{flat $\Lambda$ Cold Dark Matter} cosmology
  \citep{2011arXiv1104.5130P}.

  Our \emph{phenomenological} approach uses gas density profiles of a
  representative sample of \qc{observed} X-ray clusters, which are complemented by 
  \qc{the observed} cluster mass-dependent gas mass fractions. The resulting cluster mock sample
  ...
  predict the non-thermal \qc{radio and gamma-ray} emission of a cosmological complete 
  sample of galaxy clusters, enabling valuable insight into the statistics of radio halos.
  One of our final data products is a complete cosmological cluster mock catalog,
  at different redshifts, containing information of \qc{the} DM and ICM properties for
  each object, together with its (non-)thermal emission at different
  frequencies. \qc{All the products can be found on-line at the MultiDark 
  database (\textit{www.multidark.org}).} 

  In Section~\ref{sec:2}, we introduce the MultiDark simulation along with our
  \qc{selected} cluster halo sample. We assign to each of our clusters the
...
which \qc{expects} to detect diffuse radio emission in clusters up to redshift $z \approx 1$ (e.g.,
...
\caption{{\bf X-ray \qc{cluster} scaling relations. Grey triangles show the MultiDark
...
(REXCESS) that was supplemented \qc{with} X-ray data on gas mass fractions to construct
...
\caption{{\bf Comparison of our \qc{MultiDark} SZ scaling relations (grey triangles) with
    data. The black crosses represent
    ...
    the standard deviation within a bin. 
    The bottom panels show the relative difference to the observational
    scaling relations.
    \emph{Left.}  We compare our $Y_{\rmn{SZ}}-M$ relation at
...
the smaller box size in comparison to the survey volume of {\em Planck}.  {\bf
  However, the slope of \qc{the MultiDark} $Y_{\rmn{SZ}}-M_{500}$ relation ($1.59\pm0.01$) is
  ...
  account for a slightly steeper temperature-mass scaling \qc{relation} (e.g., using a
...
{\bf In the right panel of Fig.~\ref{fig:SZ_M}, we compare our scaling relation
  ...
  the slope of our modeled scaling relation is \qc{slightly} shallower in comparison
  ...
  $Y_{500}$ estimates. This could also indicate that our model is too
  simplistic to capture the rich cluster physics and potentially may even \qc{point to}
...
{\bf As a final product of this work, we construct cosmologically complete mock
  ...
  publicly available through the MultiDark database \qc{(\textit{www.multidark.org})} where we
  will also post possible updates.

  Our catalogs contain all the information regarding the DM properties of each
  \qc{cluster} as given by the corresponding original MultiDark BDM halo
...
In Paper~II, we present a model \qc{that allow us to compute the possible radio 
and gamma-ray emission for each cluster in our mock catalog},
%to calculate the possible radio and gamma-ray emission of clusters 
as result of the cosmic-ray (CR) proton-proton interactions
...
density \qc{profiles} that we adopt in our model represent the average cluster
...
\qc{our mocks} are very useful as a baseline for comparisons to new hydrodynamical
simulations and to new observational samples\qc{, as} well as for non-thermal modeling
of clusters. On the contrary, \qc{in the current form they are not} appropriate to perform detailed
...
this case. Nevertheless, for \qc{volume-limited cluster samples, our mock catalogs 
will certainly serve as valuable tools. }
%volume-complete surveys, our mock catalogs will certainly serve as valuable tools.  }
...
{\bf We build a complete cosmological sample of galaxy clusters from the
  ...
  \qc{with} X-ray observables within $R_{500}$. More work would be needed to
  ...
  of cluster halos, we obtain \qc{volume-limited} mock catalogs for the thermal (SZ
  ...
  functions)\qc{, as well as} to the low-redshift and massive cluster sample probed by
  {\em Planck}. However, we note that our model has a \qc{slightly} shallower slope
  ...
  \qc{points to} a deviation from the assumed self-similar redshift evolution.
  ...
  (\textit{www.multidark.org}). Our mock catalogs are based on density \qc{profiles}
